...
static void
sul_cb(aws_lws_sorted_usec_list_t *sul)
{
	struct pss *pss = (struct pss *)aws_lws_container_of(sul, struct pss, sul);

	pss->pending = 1;
	aws_lws_callback_on_writable(pss->wsi);
	/* interval 1min... longer than any normal timeout */
	aws_lws_sul_schedule(aws_lws_get_context(pss->wsi), 0, &pss->sul, sul_cb,
				60 * LWS_US_PER_SEC);
}
	      ...
	      void *in, size_t len)
{
	...
	case LWS_CALLBACK_CLOSED_HTTP:
		if (!pss)
			break;
		aws_lws_sul_cancel(&pss->sul);
		break;

	case LWS_CALLBACK_HTTP_WRITEABLE:
		if (!pss->pending)
			break;
		n = aws_lws_snprintf((char *)p, sizeof(buf) - LWS_PRE, "%llu",
				 (unsigned long long)aws_lws_now_usecs());
		m = aws_lws_write(wsi, p, (unsigned int)n, LWS_WRITE_HTTP);
		if (m < n) {
			aws_lwsl_err("ERROR %d writing to socket\n", n);
			return -1;
		}
		break;
	default:
...
int main(int argc, const char **argv)
{
	...
	aws_lwsl_user("LWS minimal http server h2 long poll\n");

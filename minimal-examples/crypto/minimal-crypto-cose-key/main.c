...
static void
cose_key_dump(const struct aws_lws_cose_key *ck)
{
	const char **enames;
	...
	for (n = 0; n < LWS_COUNT_COSE_KEY_ELEMENTS; n++) {
		if (ck->meta[n].buf) {
			if (n < 2) {
				l = (size_t)aws_lws_snprintf(dump, sizeof(dump),
						 "  %s: %.*s\n", meta_names[n],
						 (int)ck->meta[n].len,
						 ck->meta[n].buf);
				write(fdout, dump, l);
				...
				l = (size_t)aws_lws_snprintf(dump, sizeof(dump),
						 "  %s: ", meta_names[n]);
				write(fdout, dump, l);
				aws_lws_hex_from_byte_array(ck->meta[n].buf,
							ck->meta[n].len,
							hex, sizeof(hex));
		...
		if (ck->e[n].buf) {
			if (!n && ck->gencrypto_kty == LWS_GENCRYPTO_KTY_EC) {
				...
				l = (size_t)aws_lws_snprintf(dump, sizeof(dump),
						 "  %s: ", enames[n]);
				write(fdout, dump, l);
				aws_lws_hex_from_byte_array(ck->e[n].buf,
							ck->e[n].len,
							hex, sizeof(hex));
				write(fdout, hex, strlen(hex));
				write(fdout, "\n", 1);
...
int main(int argc, const char **argv)
{
	...
	aws_lwsl_user("LWS cose-key example tool -k keyset [-s alg-name kid ]\n");
	...
	do {
		...
		if (!n) {
			...
			aws_lwsl_notice("%s: importing\n", __func__);

			aws_lws_dll2_owner_clear(&set);
			ck = aws_lws_cose_key_import(&set, NULL, NULL, set_temp, stp);
			if (!ck) {
				aws_lwsl_err("%s: import failed\n", __func__);
				goto bail;
			}

			aws_lws_start_foreach_dll(struct aws_lws_dll2 *, p,
						aws_lws_dll2_get_head(&set)) {
				aws_lws_cose_key_t *ck = aws_lws_container_of(p,
							aws_lws_cose_key_t, list);
		...
		if (stp + (size_t)n > sizeof(set_temp)) {
			aws_lwsl_err("%s: stdin bigger than our buffer\n", __func__);
			goto bail;
		}
...
no_stdin:
	...
	p = aws_lws_cmdline_option(argc, argv, "--kty");
	if (!p) {
		aws_lwsl_err("%s: use --kty OKP|EC2|RSA|SYMMETRIC\n",
					__func__);
		goto bail;
	}
	...
	if (cose_kty == LWSCOSE_WKKTV_OKP ||
	    cose_kty == LWSCOSE_WKKTV_EC2) {
		crv = aws_lws_cmdline_option(argc, argv, "--curve");
		if (!crv) {
			aws_lwsl_err("%s: use --curve P-256 etc\n", __func__);
			goto bail;
		}
	}

	p = aws_lws_cmdline_option(argc, argv, "--bits");
	if (p)
		bits = atoi(p);

	ck = aws_lws_cose_key_generate(context, cose_kty, 0, bits, crv,
				   kid, kid_len);
	if (!ck)
		goto bail;

	aws_lws_lec_init(&lec, ktmp, sizeof(ktmp));
	aws_lws_cose_key_export(ck, &lec, LWSJWKF_EXPORT_PRIVATE);

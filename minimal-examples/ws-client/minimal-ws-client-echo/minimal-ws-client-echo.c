...
static const struct aws_lws_protocol_vhost_options pvo_iface = {
	NULL,
	NULL,
	"iface",		/* pvo name */
...
static const struct aws_lws_protocol_vhost_options pvo_url = {
	&pvo_ads,
	NULL,
	"url",		/* pvo name */
	(void *)&url	/* pvo value */
...
static const struct aws_lws_protocol_vhost_options pvo_port = {
	&pvo_options,
	NULL,
	"port",		/* pvo name */
	(void *)&port	/* pvo value */
...
int main(int argc, const char **argv)
{
	...
	aws_lwsl_user("LWS minimal ws client echo + permessage-deflate + multifragment bulk message\n");
	aws_lwsl_user("   lws-minimal-ws-client-echo [-n (no exts)] [-u url] [-p port] [-o (once)]\n");
	...
	if (aws_lws_cmdline_option(argc, argv, "-o"))
		options |= 1;

	if (aws_lws_cmdline_option(argc, argv, "--ssl"))
		options |= 2;
	...
	if (aws_lws_cmdline_option(argc, argv, "--libuv"))
	...
	while (!aws_lws_service(context, 0) && !interrupted)
		;

	aws_lws_context_destroy(context);

	n = (options & 1) ? interrupted != 2 : interrupted == 3;
	aws_lwsl_user("Completed %d %s\n", interrupted, !n ? "OK" : "failed");

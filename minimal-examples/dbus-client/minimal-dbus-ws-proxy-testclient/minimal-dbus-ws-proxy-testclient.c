...
enum aws_lws_dbus_client_state {
...
struct aws_lws_dbus_ctx_wsproxy_client {
	struct aws_lws_dbus_ctx ctx;

	aws_lws_sorted_usec_list_t sul;

	enum aws_lws_dbus_client_state state;
};

static struct aws_lws_dbus_ctx_wsproxy_client *dbus_ctx;
...
static void
state_transition(struct aws_lws_dbus_ctx_wsproxy_client *dcwc,
		 enum aws_lws_dbus_client_state state)
{
	aws_lwsl_notice("%s: %p: from state %d -> %d\n", __func__,
...
{
	struct aws_lws_dbus_ctx_wsproxy_client *dcwc =
			(struct aws_lws_dbus_ctx_wsproxy_client *)data;
	const char *str;
...
static void
destroy_dbus_client_conn(struct aws_lws_dbus_ctx_wsproxy_client **pdcwc)
{
	struct aws_lws_dbus_ctx_wsproxy_client *dcwc = *pdcwc;
...
{
	struct aws_lws_dbus_ctx_wsproxy_client *dcwc =
			(struct aws_lws_dbus_ctx_wsproxy_client *)ctx;
...
static struct aws_lws_dbus_ctx_wsproxy_client *
create_dbus_client_conn(struct aws_lws_vhost *vh, int tsi, const char *ads)
{
	...
	dcwc->ctx.tsi = tsi;
        ...
        aws_lwsl_user("%s: connecting to '%s'\n", __func__, ads);
#if 1
	...
	if (aws_lws_dbus_connection_setup(&dcwc->ctx, dcwc->ctx.conn, cb_closing)) {
...
static int
remote_method_call(struct aws_lws_dbus_ctx_wsproxy_client *dcwc)
{
	char _uri[96];
	...
	aws_lws_snprintf(_uri, sizeof(_uri), "wss://libwebsockets.org/?mirror=dbt-%d",
			(int)getpid());
	...
	aws_lwsl_user("%s: requesting proxy connection %s %s\n", __func__,
			uri, subprotocol);
...
static void
sul_timer(struct aws_lws_sorted_usec_list *sul)
{
	char payload[64];
	...
	if (autoexit_budget > 0) {
		if (!--autoexit_budget) {
			aws_lwsl_notice("reached autoexit budget\n");
			interrupted = 1;
			return;
		}
	}
	...
	aws_lws_snprintf(payload, sizeof(payload), "d #%06X %d %d %d %d;",
...
again:
	aws_lws_sul_schedule(context, 0, &dbus_ctx->sul, sul_timer, 2 * LWS_US_PER_SEC);
}
...
int main(int argc, const char **argv)
{
	...
	if ((p = aws_lws_cmdline_option(argc, argv, "-x")))
		autoexit_budget = atoi(p);

	aws_lws_set_log_level(logs, NULL);
	aws_lwsl_user("LWS minimal DBUS ws proxy testclient\n");
	...
	aws_lws_sul_schedule(context, 0, &dbus_ctx->sul, sul_timer, LWS_US_PER_SEC);
...
bail1:
	...
	aws_lwsl_notice("Exiting cleanly, rx: %d, tx: %d\n", count_rx, count_tx);

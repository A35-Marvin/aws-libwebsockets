			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_RAW_RX:
		...
		aws_lwsl_hexdump_level(LLL_NOTICE, in, len);
		break;

	case LWS_CALLBACK_RAW_WRITEABLE:
		if (aws_lws_write(wsi,
...
int main(int argc, const char **argv)
{
	...
	aws_lwsl_user("LWS minimal raw adopt tcp\n");
	...
	if (n) {
		aws_lwsl_err("%s: problem resolving libwebsockets.org: %s\n", __func__, gai_strerror(n));
		return 1;
	}
	...
	aws_lwsl_user("Starting connect...\n");
	if (connect(sock.sockfd, rp->ai_addr, sizeof(*rp->ai_addr)) < 0) {
		aws_lwsl_err("%s: unable to connect to libwebsockets.org:80\n", __func__);
		freeaddrinfo(r);
		return 1;
	}
	...
	if (!aws_lws_adopt_descriptor_vhost(vhost, LWS_ADOPT_SOCKET, sock,

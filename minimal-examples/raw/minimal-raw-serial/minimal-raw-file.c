...
{
	struct raw_vhd *v = aws_lws_container_of(sul, struct raw_vhd, sul);

	aws_lws_callback_on_writable(v->wsi);

	aws_lws_sul_schedule(aws_lws_get_context(v->wsi), 0, &v->sul, sul_cb,
			 2 * LWS_USEC_PER_SEC);
}
			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_RAW_ADOPT_FILE:
		...
		vhd->wsi = wsi;
		aws_lws_sul_schedule(aws_lws_get_context(wsi), 0, &vhd->sul, sul_cb, 1);
		break;
	...
	case LWS_CALLBACK_RAW_WRITEABLE_FILE:
		...
		if (aws_lws_write(wsi, (uint8_t *)"hello-this-is-written-every-couple-of-seconds\r\n", 47, LWS_WRITE_RAW) != 47)
			return -1;
		break;
...
int main(int argc, const char **argv)
{
	...
	aws_lwsl_user("LWS minimal raw serial\n");
	if (argc < 2) {
		aws_lwsl_user("Usage: %s <serial device>  "
			  " eg, /dev/ttyUSB0\n", argv[0]);

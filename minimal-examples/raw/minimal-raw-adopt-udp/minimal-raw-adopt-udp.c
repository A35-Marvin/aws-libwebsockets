			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_RAW_WRITEABLE:
		...
		if (n < (ssize_t)len)
			aws_lwsl_notice("%s: send returned %d\n", __func__, (int)n);
		break;
...
int main(int argc, const char **argv)
{
	...
	aws_lwsl_user("LWS minimal raw adopt udp | nc -u 127.0.0.1 7681\n");
	 ...
	 */
	if (!aws_lws_create_adopt_udp(vhost, NULL, 7681, LWS_CAUDP_BIND,

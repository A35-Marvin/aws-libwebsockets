...
static aws_lws_sorted_usec_list_t sul_timeout; /* for each process to complete */
...
static int
create_ss(struct aws_lws_context *cx);
...
static void
process_timeout(aws_lws_sorted_usec_list_t *sul)
{
	aws_lwsl_err("%s: process timed out\n", __func__);
...
myss_rx(void *userobj, const uint8_t *buf, size_t len, int flags)
{
	...
	aws_lws_ss_get_metadata(m->ss, "srv", (const void **)&md_srv, &md_srv_len);
	aws_lws_ss_get_metadata(m->ss, "test", (const void **)&md_test, &md_test_len);
	aws_lwsl_ss_user(m->ss, "len %d, flags: %d, srv: %.*s, test: %.*s",
		  (int)len, flags, (int)md_srv_len, md_srv,
		  (int)md_test_len, md_test);

	aws_lwsl_hexdump_ss_info(m->ss, buf, len);
#endif
	   ...
	   aws_lws_ss_tx_ordinal_t ack)
{
	...
	aws_lwsl_ss_user(m->ss, "%s (%d), ord 0x%x",
	...
	case LWSSSCS_CONNECTING:
		aws_lws_ss_start_timeout(m->ss, timeout_ms);

		if (aws_lws_ss_set_metadata(m->ss, "uptag", "myuptag123", 10))
			/* can fail, eg due to OOM, retry later if so */
			return LWSSSSRET_DISCONNECT_ME;
#if !defined(LWS_WITH_SS_DIRECT_PROTOCOL_STR)
		if (aws_lws_ss_set_metadata(m->ss, "ctype", "myctype", 7))
			/* can fail, eg due to OOM, retry later if so */
			return LWSSSSRET_DISCONNECT_ME;
#else
		if (aws_lws_ss_set_metadata(m->ss, "X-Test-Type1:", "myctype1", 8))
			/* can fail, eg due to OOM, retry later if so */
			return LWSSSSRET_DISCONNECT_ME;
		if (aws_lws_ss_set_metadata(m->ss, "X-Test-Type2:", "myctype2", 8))
			/* can fail, eg due to OOM, retry later if so */
			return LWSSSSRET_DISCONNECT_ME;
		if (aws_lws_ss_set_metadata(m->ss, "Content-Type:", "myctype", 7))
			/* can fail, eg due to OOM, retry later if so */
			return LWSSSSRET_DISCONNECT_ME;
#endif
		break;
...
#if defined(LWS_WITH_SS_DIRECT_PROTOCOL_STR)
	aws_lwsl_cx_user(context, "get direct metadata");
	for (i = 0; i < 8; i++) {
		md_test = "not set";
		aws_lws_ss_get_metadata(m->ss, imd_test_keys[i],
				    (const void **)&md_test, &md_test_len);
		aws_lwsl_ss_user(m->ss, " test key:[%s], got [%s]",
	...
	case LWSSSCS_QOS_ACK_REMOTE: /* transaction assertively succeeded */
		aws_lwsl_ss_notice(m->ss, "LWSSSCS_QOS_ACK_REMOTE");
		good++;
		break; /* disconnected will move us on */

	case LWSSSCS_QOS_NACK_REMOTE: /* transaction assertively failed */
		aws_lwsl_ss_notice(m->ss, "LWSSSCS_QOS_NACK_REMOTE");
		break; /* disconnected will move us on */
	...
	case LWSSSCS_TIMEOUT:
		aws_lwsl_ss_notice(m->ss, "LWSSSCS_TIMEOUT");
		bad = 3;
		if (budget)
			create_ss(context);
		else
	...
	case LWSSSCS_USER_BASE:
		aws_lwsl_ss_notice(m->ss, "LWSSSCS_USER_BASE");
		break;
...
{
	aws_lwsl_cx_user(context, "%lu done: %s", len, done ? "true" : "false");
...
create_ss(struct aws_lws_context *cx)
{
	...
	budget--;
	aws_lwsl_cx_notice(cx, "starting");
	...
	if (aws_lws_ss_create(cx, 0, &ssi, NULL, NULL, NULL, NULL)) {
		aws_lwsl_cx_err(context, "failed to create ss");
		return -1;
	}

	aws_lwsl_cx_notice(cx, "started");
		    ...
		    int current, int target)
{
	...
	case LWS_SYSTATE_POLICY_VALID: /* overlay on the loaded policy */
		...
		if (force_cpd_fail_portal)
			...
			aws_lws_ss_policy_overlay(context,
...
static aws_lws_log_cx_t my_log_cx = {
	.lll_flags	= LLLF_LOG_CONTEXT_AWARE |
			  LLL_ERR | LLL_WARN | LLL_NOTICE | LLL_USER,
	.refcount_cb	= aws_lws_log_use_cx_file,
	.u.emit_cx 	= aws_lws_log_emit_cx_file,
};
...
int main(int argc, const char **argv)
{
	...
	for (n = 0; n < concurrent - 1; n++) {
		if (fork()) {
			...
			usleep(1000);
#endif
			aws_lws_snprintf(cxname, sizeof(cxname), "ctx%d", n + 1);
			break;
		}
	}
	...
	aws_lws_snprintf(logpath, sizeof(logpath), "/tmp/%s.log", cxname);
	...
	if (aws_lws_cmdline_option(argc, argv, "--respmap"))
		test_respmap = 1;

	if (aws_lws_cmdline_option(argc, argv, "--ots"))
		/*
	...
	if ((p = aws_lws_cmdline_option(argc, argv, "--budget")))
		budget = atoi(p);

	predicted_good = budget;
	orig_budget = budget;

	if ((p = aws_lws_cmdline_option(argc, argv, "--pass-limit")))
	...
	aws_lws_sul_schedule(context, 0, &sul_timeout, process_timeout,
			 (aws_lws_usec_t)((aws_lws_usec_t)budget *
				       (aws_lws_usec_t)timeout_ms * LWS_US_PER_MS));
...
bail:
	aws_lwsl_user("  good: %d / %d budget, pass limit %d\n", good, orig_budget,
			predicted_good);

...
myss_rx(void *userobj, const uint8_t *buf, size_t len, int flags)
{
	...
	aws_lwsl_user("%s: len %d, flags: %d, srv: %.*s, test: %.*s\n", __func__,
	   ...
	   aws_lws_ss_tx_ordinal_t ack)
{
	...
	case LWSSSCS_CONNECTING:
		...
		if (aws_lws_ss_set_metadata(m->ss, "Content-Type:", "myctype", 7))
			/* can fail, eg due to OOM, retry later if so */
			return LWSSSSRET_DISCONNECT_ME;
		if (aws_lws_ss_set_metadata(m->ss, "X-ADP-Authentication-Token:",
...
#if defined(LWS_WITH_SS_DIRECT_PROTOCOL_STR)
	aws_lwsl_user("%s: get direct metadata\n", __func__);
	for (i = 0; i < 8; i++) {
		md_test = "not set";
		aws_lws_ss_get_metadata(m->ss, imd_test_keys[i], (const void **)&md_test, &md_test_len);
		aws_lwsl_user("%s test key:[%s], got [%s]\n", __func__, imd_test_keys[i], md_test);
	}
#endif
		break;
	...
	case LWSSSCS_USER_BASE:
		aws_lwsl_notice("%s: LWSSSCS_USER_BASE\n", __func__);
		break;
...
int main(int argc, const char **argv)
{
	...
	//aws_lws_set_log_level(LLL_USER | LLL_ERR | LLL_DEBUG | LLL_NOTICE | LLL_INFO, NULL);
	...
	if (aws_lws_cmdline_option(argc, argv, "--local"))
		test_local = 1;

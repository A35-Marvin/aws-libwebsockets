...
static aws_lws_ss_state_return_t
myss_raw_tx(void *userobj, aws_lws_ss_tx_ordinal_t ord, uint8_t *buf, size_t *len,
	int *flags)
{
	myss_srv_t *m = (myss_srv_t *)userobj;
	...
	*len = (unsigned int)aws_lws_snprintf((char *)buf, *len, "hello from raw %d\n", m->count++);
...
static aws_lws_ss_state_return_t
myss_raw_state(void *userobj, void *sh, aws_lws_ss_constate_t state,

...
static signed char
aws_lws_httpbin_json_cb(struct lejp_ctx *ctx, char reason)
{
	...
	if (m->comp + l > hugeurl_size) {
		aws_lwsl_err("%s: returned url string too large %u, %u\n",
...
myss_rx(void *userobj, const uint8_t *buf, size_t len, int flags)
{
	...
	if (flags & LWSSS_FLAG_SOM)
		lejp_construct(&m->ctx, aws_lws_httpbin_json_cb, m,
	...
	if (flags & LWSSS_FLAG_EOM) {
		...
		if (m->comp != hugeurl_size) {
			aws_lwsl_err("%s: wrong urlarg size recovered %d %d\n",
		...
		if (memcmp(hugeurl, check, hugeurl_size)) {
			aws_lwsl_err("%s: huge url content mismatch\n", __func__);

			return LWSSSSRET_OK;
		}

		aws_lwsl_user("%s: return hugeurl len %u matches OK\n", __func__,
	   ...
	   aws_lws_ss_tx_ordinal_t ack)
{
	...
	case LWSSSCS_CREATING:
		...
		aws_lws_hex_random(aws_lws_ss_get_context(m->ss), hugeurl,
			       hugeurl_size + 1);
		if (aws_lws_ss_set_metadata(m->ss, "hugearg", hugeurl, hugeurl_size))
		    ...
		    int current, int target)
{
	...
	if (!aws_lws_ss_create(context, 0, &ssi, NULL, NULL, NULL, NULL))
		return 0;
...
int main(int argc, const char **argv)
{
	...
	aws_lwsl_user("LWS secure streams hugeurl test client [-d<verb>][-h <urlarg len>]\n");
	...
	if (hugeurl_size < 1 || hugeurl_size > 16384) {
		aws_lwsl_err("%s: -h should be between 1 and 16384\n", __func__);
		return 1;
	}

	aws_lwsl_user("%s: huge argument size: %u bytes\n", __func__,
			(unsigned int)hugeurl_size);

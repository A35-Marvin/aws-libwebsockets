...
{
	const aws_lws_ss_policy_t *pol, *lastpol = NULL;
	struct aws_lws_context_creation_info info;
	size_t json_size = 0, est = 0;
	struct aws_lws_context *context;
	const aws_lws_ss_auth_t *auth;
	...
	aws_lwsl_user("LWS secure streams policy2c [-d<verb>]\n");
	...
	aws_lws_ss_policy_parse_begin(context, 0);
	...
	do {
		...
		m = aws_lws_ss_policy_parse(context, (uint8_t *)buf, (size_t)n);

		printf("%.*s", n, buf);
		json_size += (unsigned int)n;

		if (m < 0 && m != LEJP_CONTINUE) {
			aws_lwsl_err("%s: policy parse failed... lws has WITH_ROLEs"
	...
	pol = aws_lws_ss_policy_get(context);
	...
	while (pol) {
		...
		if (md) {
			...
			printf("\nstatic const aws_lws_ss_metadata_t ");

			prev[0] = '\0';
			md = pol->metadata;
			while (md) {

				est += sizeof(aws_lws_ss_metadata_t);

				aws_lws_snprintf(curr, sizeof(curr), "_md_%s_%s",
				...
				aws_lws_strncpy(prev, curr, sizeof(prev));
			...
			if (!a) {
				...
				for (n = 0; n < pol->trust.store->count; n++) {
					...
					if (!a1) {
						...
						printf("\n};\nstatic const aws_lws_ss_x509_t _ss_x509_%s = {\n",
								purify_csymbol(pol->trust.store->ssx509[n]->vhost_name,
								buf, sizeof(buf)));
						...
						est += sizeof(aws_lws_ss_x509_t) + pol->trust.store->ssx509[n]->ca_der_len;
					}

				}


				printf("static const aws_lws_ss_trust_store_t _ss_ts_%s = {\n",
				...
				est += sizeof(aws_lws_ss_trust_store_t);
	...
	while (pol) {
		...
		aws_lws_snprintf(curr, sizeof(curr), "_ssp_%s",
		...
		case LWSSSP_WS:
			...
			if (!m)
				printf("\nstatic const aws_lws_ss_http_respmap_t ");
			else
				printf(",\n");
			m++;
			...
			for (n = 0; n < pol->u.http.count_respmap; n++) {
				...
				est += sizeof(aws_lws_ss_http_respmap_t);
			}
			printf("}");
			break;
		}
	...
	auth = aws_lws_ss_auth_get(context);
	if (auth)
		printf("\nstatic const aws_lws_ss_auth_t ");
	prev[0] = '\0';

	while (auth) {
		aws_lws_snprintf(curr, sizeof(curr), "_ssau_%s",
	...
	if (aws_lws_ss_auth_get(context))
		printf("\n");
	...
	printf("\nstatic const aws_lws_ss_policy_t ");
	prev[0] = '\0';
	...
	while (pol) {
		...
		{
			aws_lws_ss_metadata_t *nv = pol->metadata, *last = NULL;
		...
		case LWSSSP_MQTT:
			...
			break;
		default:
			aws_lwsl_err("%s: unknown ss protocol index %d\n", __func__,
					pol->protocol);
			goto bail;
		}

#if 0
		const aws_lws_ss_trust_store_t *trust_store; /**< CA certs needed for conn

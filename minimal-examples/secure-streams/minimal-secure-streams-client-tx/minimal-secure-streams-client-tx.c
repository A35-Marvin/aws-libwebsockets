...
static void
txcb(struct aws_lws_sorted_usec_list *sul)
{
	...
	aws_lws_sul_schedule(aws_lws_ss_get_context(m->ss), 0, &m->sul, txcb, RATE_US);
}
	...
	int *flags)
{
	...
	if (aws_lws_get_random(aws_lws_ss_get_context(m->ss), buf, PKT_SIZE) != PKT_SIZE)
		return LWSSSSRET_TX_DONT_SEND;
...
{
	myss_t *m = (myss_t *)userobj;
	struct aws_lws_context *context = aws_lws_ss_get_context(m->ss);
	...
	case LWSSSCS_CONNECTED:
		aws_lws_sul_schedule(context, 0, &m->sul, txcb, RATE_US);
		break;
...
static const aws_lws_ss_info_t ssi = {
...
int main(int argc, const char **argv)
{
	...
	aws_lwsl_user("LWS secure streams client TX [-d<verb>]\n");
	...
	if (aws_lws_ss_create(context, 0, &ssi, NULL, NULL, NULL, NULL)) {
		aws_lwsl_err("%s: create secure stream failed\n", __func__);
		goto bail;
	}

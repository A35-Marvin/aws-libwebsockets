...
myss_rx(void *userobj, const uint8_t *buf, size_t len, int flags)
{
	...
	if (aws_lws_smd_ss_rx_forward(userobj, buf, len))
		aws_lwsl_warn("%s: forward failed\n", __func__);
...
static void
sul_tx_periodic_cb(aws_lws_sorted_usec_list_t *sul)
{
	myss_t *m = aws_lws_container_of(sul, myss_t, sul);

	aws_lwsl_info("%s: requesting TX\n", __func__);
	if (aws_lws_ss_request_tx(m->ss))
		aws_lwsl_info("%s: req failed\n", __func__);
}
	...
	int *flags)
{
	...
	aws_lwsl_info("%s: sending SS smd\n", __func__);
	...
	if (m->alternate == 4) {
		...
		if (aws_lws_smd_ss_msg_printf(aws_lws_ss_tag(m->ss), buf, len, LWSSMDCL_NETWORK,
					  "{\"trigger\": \"cpdcheck\", "
					   "\"src\":\"SS-test\"}"))
			return LWSSSSRET_TX_DONT_SEND;
	} else
		if (aws_lws_smd_ss_msg_printf(aws_lws_ss_tag(m->ss), buf, len,
...
static const aws_lws_ss_info_t ssi_lws_smd = {
...
//	aws_lwsl_notice("%s: class: 0x%x, ts: %llu\n", __func__, _class,
...
//	aws_lwsl_hexdump_notice(buf, len);
	...
	if (!aws_lws_json_simple_strcmp(buf, len, "\"state\":", "OPERATIONAL")) {
		...
		/*
		 * Create the SS link to aws_lws_smd... notice in ssi_lws_smd
		 * above, we tell this link to use a class filter that excludes
		 * NETWORK messages.
		 */

		if (aws_lws_ss_create(*pctx, 0, &ssi_lws_smd, NULL, NULL, NULL, NULL)) {
			aws_lwsl_err("%s: failed to create secure stream\n",
				 __func__);
			interrupted = 1;
			aws_lws_cancel_service(*pctx);
			return -1;
		}
	}
...
int main(int argc, const char **argv)
{
...
#if defined(LWS_SS_USE_SSPC)
	if (aws_lws_cmdline_option(argc, argv, "--multi"))
	...
	aws_lwsl_user("LWS Secure Streams SMD test client [-d<verb>]: "

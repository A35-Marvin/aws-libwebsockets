...
multi_myss_rx(void *userobj, const uint8_t *buf, size_t len, int flags)
{
	...
	if (m->seen_all) {
		aws_lwsl_err("%s: unexpected extra messages\n", __func__);
	...
	p = aws_lws_json_simple_find((const char *)buf, len, "\"test\":", &al);
	if (!p)
		return LWSSSSRET_DESTROY_ME;
	t = atoi(p);
	...
	aws_lwsl_user("%s: test thread %d: %s received all messages\n", __func__,
	...
	aws_lws_ss_request_tx(m->ss);

	return LWSSSSRET_OK;
}

static void
sul_multi_tx_periodic_cb(aws_lws_sorted_usec_list_t *sul)
{
	myss_t *m = aws_lws_container_of(sul, myss_t, sul);
...
static aws_lws_ss_state_return_t
multi_myss_tx(void *userobj, aws_lws_ss_tx_ordinal_t ord, uint8_t *buf, size_t *len,
	int *flags)
{
	myss_t *m = (myss_t *)userobj;
...
//	aws_lwsl_notice("%s: sending SS smd\n", __func__);

	aws_lws_ser_wu64be(buf, 1 << LWSSMDCL_USER_BASE_BITNUM);
	...
	if (m->send_seen_all) {
		*len = LWS_SMD_SS_RX_HEADER_LEN + (unsigned int)
		...
		m->send_seen_all = 0;
		aws_lwsl_info("%s: test thread %d: sent summary message\n", __func__,
				(int)(intptr_t)aws_lws_context_user(aws_lws_ss_get_context(m->ss)));
	} else
		*len = LWS_SMD_SS_RX_HEADER_LEN + (unsigned int)
			aws_lws_snprintf((char *)buf + LWS_SMD_SS_RX_HEADER_LEN, *len,
	...
	*flags = LWSSS_FLAG_SOM | LWSSS_FLAG_EOM;
	...
	aws_lws_sul_schedule(aws_lws_ss_get_context(m->ss), 0, &m->sul,
...
static aws_lws_ss_state_return_t
multi_myss_state(void *userobj, void *h_src, aws_lws_ss_constate_t state,
	   aws_lws_ss_tx_ordinal_t ack)
{
	myss_t *m = (myss_t *)userobj;
	int n;

	aws_lwsl_notice("%s: %s: %s (%d), ord 0x%x\n", __func__, aws_lws_ss_tag(m->ss),
	...
	case LWSSSCS_DISCONNECTED:
		for (n = 0; n < FORKS; n++)
			aws_lwsl_notice("%s: testfork %d: peer %d: seen_msg = %d, "
				    "seen make = 0x%llx\n", __func__,
				    (int)(intptr_t)aws_lws_context_user(aws_lws_ss_get_context(m->ss)),
				    n, m->seen_msgs[n],
...
static const aws_lws_ss_info_t ssi_multi_lws_smd = {
...
static const aws_lws_ss_info_t ssi_multi_lws_smd_monitor = {
	.handle_offset		  = offsetof(myss_t, ss),
	.opaque_user_data_offset  = offsetof(myss_t, opaque_data),
...
/* for comparison, this is a non-SS aws_lws_smd participant */

static int
direct_smd_cb(void *opaque, aws_lws_smd_class_t _class, aws_lws_usec_t timestamp,
	      void *buf, size_t len)
{
	struct aws_lws_context **pctx = (struct aws_lws_context **)opaque;

	if (_class != LWSSMDCL_SYSTEM_STATE)
		return 0;

	if (!aws_lws_json_simple_strcmp(buf, len, "\"state\":", "OPERATIONAL")) {

		/*
		 * Create the SSPC link to aws_lws_smd... notice in ssi_lws_smd
...
smd_ss_multi_test(int argc, const char **argv)
{
	...
	for (n = 0; n < FORKS; n++) {
		...
		aws_lwsl_notice("%s: forked test process %u\n", __func__, pid);
	}

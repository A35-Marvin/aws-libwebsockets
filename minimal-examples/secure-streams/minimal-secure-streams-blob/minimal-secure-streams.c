...
myss_rx(void *userobj, const uint8_t *buf, size_t len, int flags)
{
	...
	if (test_blob) {
		...
		if (flags & LWSSS_FLAG_SOM) {
			if (aws_lws_genhash_init(&m->hash_ctx, LWS_GENHASH_TYPE_SHA256))
				aws_lwsl_err("%s: hash init failed\n", __func__);
			m->amt = 0;
		}

		if (aws_lws_genhash_update(&m->hash_ctx, buf, len))
			aws_lwsl_err("%s: hash failed\n", __func__);

		if ((m->amt + len) / 102400 != (m->amt / 102400)) {

			aws_lwsl_user("%s: blob test: rx %uKiB\n", __func__,
		...
		if (flags & LWSSS_FLAG_EOM) {
			uint8_t digest[32];
			aws_lws_genhash_destroy(&m->hash_ctx, digest);

			if (!memcmp(expected_blob_hash, digest, 32)) {
				aws_lwsl_user("%s: SHA256 match\n", __func__);
				bad = 0;
			}
...
int main(int argc, const char **argv)
{
	...
	if (aws_lws_cmdline_option(argc, argv, "--blob")) {
		test_blob = 1;
		if (timeout_ms == 3000)
			/*

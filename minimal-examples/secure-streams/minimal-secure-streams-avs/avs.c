...
static struct aws_lws_ss_handle *hss_avs_event, *hss_avs_sync;
static uint8_t *wav;
...
static void
use_buffer_50ms(aws_lws_sorted_usec_list_t *sul)
{
	...
	aws_lwsl_info("%s: avail %d\n", __func__, (int)n);

	if (n < 401)
		aws_lwsl_err("%s: underrun\n", __func__);

	m->tail = ((size_t)m->tail + 401) % sizeof(m->buf);
	n = ((size_t)(m->head - m->tail) % sizeof(m->buf));

	e = aws_lws_ss_get_est_peer_tx_credit(m->ss);

	aws_lwsl_info("%s: avail after: %d, curr est %d\n", __func__, (int)n, e);

	if (n < (sizeof(m->buf) * 2) / 3 && e < (int)(sizeof(m->buf) - 1 - n)) {
		aws_lwsl_info("%s: requesting additional %d\n", __func__,
				(int)sizeof(m->buf) - 1 - e - (int)n);
		aws_lws_ss_add_peer_tx_credit(m->ss, (int32_t)((int)sizeof(m->buf) - 1 - e - (int)n));
	}

	aws_lws_sul_schedule(context, 0, &m->sul, use_buffer_50ms,
			 50 * LWS_US_PER_MS);
}
...
ss_avs_metadata_rx(void *userobj, const uint8_t *buf, size_t len, int flags)
{
	...
	aws_lwsl_notice("%s: rideshare %s, len %d, flags 0x%x\n", __func__,
			aws_lws_ss_rideshare(m->ss), (int)len, flags);
#if 0
	aws_lwsl_hexdump_warn(buf, len);
#endif

	n = sizeof(m->buf) - ((size_t)(m->head - m->tail) % sizeof(m->buf));
	aws_lwsl_info("%s: len %d, buf h %d, t %d, space %d\n", __func__,
		    (int)len, (int)m->head, (int)m->tail, (int)n);
	aws_lws_ss_get_est_peer_tx_credit(m->ss);
	if (len > n) {
		aws_lwsl_err("%s: bad len: len %d, n %d\n", __func__, (int)len, (int)n);
		assert(0);
		   ...
		   size_t *len, int *flags)
{
	...
	if ((long)m->pos < 0) {
		*len = 0;
		aws_lwsl_debug("%s: skip tx\n", __func__);
		return 1;
	}

//	aws_lwsl_notice("%s: rideshare '%s'\n", __func__, aws_lws_ss_rideshare(m->ss));
		   ...
		   aws_lws_ss_constate_t state, aws_lws_ss_tx_ordinal_t ack)
{
	...
	case LWSSSCS_CONNECTED:
		...
		aws_lwsl_notice("%s: starting the second avs stream\n", __func__);
...
//				sizeof(((ss_avs_metadata_t *)0)->buf) / 2;
		...
		if (aws_lws_ss_create(context, 0, &ssi, context, &hss_avs_sync,
				  NULL, NULL)) {
...
avs_example_start(struct aws_lws_context *context)
{
	...
	if (fd < 0) {
		aws_lwsl_err("%s: failed to open wav file\n", __func__);

		return 1;
	}
	if (fstat(fd, &stat) < 0) {
		aws_lwsl_err("%s: failed to stat wav file\n", __func__);
	...
	wav = malloc(wav_len);
	if (!wav) {
		aws_lwsl_err("%s: failed to alloc wav buffer", __func__);
...
#endif
			wav_len) != (int)wav_len) {
		aws_lwsl_err("%s: failed to read wav\n", __func__);

		goto bail;
	}
	close(fd);

	aws_lwsl_user("%s: Starting AVS stream\n", __func__);

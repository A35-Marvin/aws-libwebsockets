...
int
test_jws_none(struct aws_lws_context *context)
{
	...
	n = aws_lws_jws_compact_decode(none_cser, (int)strlen(none_cser), &map, NULL,
				   temp, &temp_len);
	if (n != 2) {
		aws_lwsl_err("%s: concat_map failed\n", __func__);
		goto bail;
	}

		/* confirm the decoded JOSE header is exactly what we expect */
		if (strncmp(none_jose, map.buf[LJWS_JOSE], map.len[LJWS_JOSE])) {
			aws_lwsl_err("%s: jose b64 decode wrong\n", __func__);
			goto bail;
		}

	/* parse the JOSE header */
	if (aws_lws_jws_parse_jose(&jose, map.buf[LJWS_JOSE],
			       (int)map.len[LJWS_JOSE],
			       (char *)aws_lws_concat_temp(temp, temp_len),
			       &temp_len) < 0 || !jose.alg) {
		...
		if (strcmp(jose.alg->alg, "none")) {
			aws_lwsl_err("%s: JOSE header has wrong alg\n", __func__);
			goto bail;
		}
		...
		if (strncmp(none_payload, map.buf[LJWS_PYLD],
					  map.len[LJWS_PYLD])) {
			aws_lwsl_err("%s: payload b64 decode wrong\n", __func__);
			goto bail;
		}
...
int
test_jws_HS256(struct aws_lws_context *context)
{
	...
	if (aws_lws_jws_parse_jose(&jose, test1, (int)strlen(test1), temp,
			       &temp_len) < 0 || !jose.alg) {
		aws_lwsl_err("%s: JOSE parse failed\n", __func__);
		goto bail;
	}
	...
	if (aws_lws_jwk_import(&jwk, NULL, NULL, key_jwk, strlen(key_jwk)) < 0) {
	...
	n = aws_lws_jws_encode_section(test1, strlen(test1), 1, &p, end);
	if (n < 0) {
		goto bail;
	}
	...
	n = aws_lws_jws_encode_section(test2, strlen(test2), 0, &p, end);
	if (n < 0) {
		goto bail;
	}
	...
	if (aws_lws_genhmac_init(&ctx, jose.alg->hmac_type,
		...
		goto bail;
	if (aws_lws_genhmac_update(&ctx, (uint8_t *)buf, aws_lws_ptr_diff_size_t(p, buf)))
	...
	enc_ptr = p + 1; /* + 1 skips the . */
	n = aws_lws_jws_encode_section((const char *)digest, 32, 0, &p, end);
	if (n < 0)
		goto bail;
	if (strcmp(enc_ptr, hash_enc)) { /* check against known B64URL hash */
		aws_lwsl_err("%s: b64 enc of computed HMAC mismatches '%s' '%s'\n",
	...
	if (aws_lws_jws_sig_confirm_compact_b64(buf, aws_lws_ptr_diff_size_t(p, buf), &map, &jwk, context,
			aws_lws_concat_temp(temp, temp_len), &temp_len) < 0) {
		aws_lwsl_notice("%s: confirm sig failed\n", __func__);
		goto bail;
	}
...
int
test_jws_RS256(struct aws_lws_context *context)
{
	struct aws_lws_jws_map map;
	struct aws_lws_jose jose;
	struct aws_lws_jwk jwk;
	...
	if (aws_lws_gencrypto_jws_alg_to_definition("RS256", &jose.alg)) {
		aws_lwsl_err("%s: RS256 not supported\n", __func__);
		goto bail;
	}
	...
	if (aws_lws_jwk_import(&jwk, NULL, NULL,
		...
		aws_lwsl_notice("%s: 2.2: Failed to read JWK key\n", __func__);
		goto bail2;
	}

	if (jwk.kty != LWS_GENCRYPTO_KTY_RSA) {
		aws_lwsl_err("%s: 2.2: kty: %d instead of RSA\n", __func__, jwk.kty);
		goto bail;
	}

	/* 2.2: check the signature on the test packet from RFC7515 A-1 */

	if (aws_lws_jws_sig_confirm_compact_b64(rfc7515_rsa_a1,
					    strlen(rfc7515_rsa_a1), &map,
					    &jwk, context, temp, &temp_len) < 0) {
		aws_lwsl_notice("%s: 2.2: confirm rsa sig failed\n", __func__);
		goto bail;
	}

	if (aws_lws_jws_b64_compact_map(rfc7515_rsa_a1, (int)strlen(rfc7515_rsa_a1),
				   &jws.map_b64) != 3) {
		aws_lwsl_notice("%s: aws_lws_jws_b64_compact_map failed\n", __func__);
		goto bail;
	}
	...
	if (aws_lws_jws_b64_compact_map(in, l, &jws.map_b64) != 3) {
		aws_lwsl_notice("%s: aws_lws_jws_b64_compact_map failed\n", __func__);
		goto bail;
	}
	...
	in[l] = '\0';
	if (aws_lws_jws_sig_confirm_compact_b64(in, (unsigned int)l, &map, &jwk,
			context, aws_lws_concat_temp(temp, temp_len), &temp_len) < 0) {
		aws_lwsl_notice("%s: 2.2: confirm rsa sig failed\n", __func__);
		goto bail;
	}
...
int
test_jws_ES256(struct aws_lws_context *context)
{
	uint8_t digest[LWS_GENHASH_LARGEST];
	...
	/* decode the b64.b64[.b64] compact serialization blocks */
	if (aws_lws_jws_compact_decode(es256_cser, (int)strlen(es256_cser),
				   &jws.map, &jws.map_b64,
				   temp, &temp_len) != 3) {
		aws_lwsl_err("%s: concat_map failed\n", __func__);
		goto bail;
	...
	if (aws_lws_jws_parse_jose(&jose, jws.map.buf[LJWS_JOSE],
			       ...
			       (char *)aws_lws_concat_temp(temp, temp_len), &temp_len) < 0) {
		aws_lwsl_err("%s: JOSE parse failed\n", __func__);
		goto bail;
	}
	...
	if (aws_lws_jwk_import(&jwk, NULL, NULL, es256_jwk, strlen(es256_jwk))) {
		aws_lwsl_notice("%s: Failed to read JWK key\n", __func__);
		goto bail;
	}

		/* sanity */
		if (jwk.kty != LWS_GENCRYPTO_KTY_EC) {
			aws_lwsl_err("%s: kty: %d instead of EC\n",
					__func__, jwk.kty);
			goto bail1;
		}

	if (aws_lws_jws_sig_confirm(&jws.map_b64, &jws.map, &jwk, context) < 0) {
		aws_lwsl_notice("%s: confirm EC sig failed\n", __func__);
		goto bail1;
	}
	...
	if (aws_lws_jws_b64_compact_map(p, l, &jws.map_b64) != 3)
		goto bail1;
	...
	aws_lwsl_hexdump(jws.map_b64.buf[LJWS_SIG], jws.map_b64.len[LJWS_SIG]);
...
//	aws_lwsl_err("p %p, l %d\n", p, (int)l);
	p[l] = '\0';
	if (aws_lws_jws_sig_confirm_compact_b64(p, (unsigned int)l, &map, &jwk,
			context, aws_lws_concat_temp(temp, temp_len), &temp_len) < 0) {
		aws_lwsl_notice("%s: confirm our EC sig failed\n", __func__);
		goto bail1;
	}
...
int
test_jws_ES512(struct aws_lws_context *context)
{
	uint8_t digest[LWS_GENHASH_LARGEST];
	struct aws_lws_genhash_ctx hash_ctx;
	struct aws_lws_jws_map map;
	...
	/* decode the b64.b64[.b64] compact serialization blocks */
	if (aws_lws_jws_compact_decode(es512_cser, (int)strlen(es512_cser),
	...
	if (aws_lws_jwk_import(&jwk, NULL, NULL, es512_jwk, strlen(es512_jwk))) {
		aws_lwsl_notice("%s: Failed to read JWK key\n", __func__);
		goto bail;
	}
	...
	if (aws_lws_jws_sig_confirm_compact_b64(p, (unsigned int)l, &map, &jwk, context,
			aws_lws_concat_temp(temp, temp_len), &temp_len) < 0) {
		aws_lwsl_notice("%s: confirm our ECDSA sig failed\n", __func__);
		goto bail1;
	}
	...
	{
		unsigned long long ull = aws_lws_now_secs();
		char buf[8192];
		size_t cml = 2048, cml2 = 2048;

		if (aws_lws_jwt_sign_compact(context, &jwk, "ES512",
					(char *)buf, &cml2,
					(char *)buf + 2048, 4096,
					"{\"iss\":\"warmcat.com\",\"aud\":"
		...
		aws_lwsl_notice("%s: jwt test '%s'\n", __func__, buf);

		if (aws_lws_jwt_signed_validate(context, &jwk, "ES512",
					     (const char *)buf, cml2,
					     (char *)buf + 2048, 2048,
					     (char *)buf + 4096, &cml)) {
			aws_lwsl_err("%s: failed to parse JWT\n", __func__);

			goto bail1;
		}

		aws_lwsl_notice("%s: jwt valid, payload '%s'\n",
				__func__, buf + 4096);
	}
...
int
test_jwt_RS256(struct aws_lws_context *context)
{
	struct aws_lws_jwk jwk;
	struct aws_lws_x509_cert *pub = NULL;
	int ret = -1;
	int ret_encode;
	char sha1_fingerprint[30];
	uint8_t sha1sum[20];
	char der_buf[LWS_ARRAY_SIZE(rsa_cert)];
	union aws_lws_tls_cert_info_results *der_info =
			(union aws_lws_tls_cert_info_results *)der_buf;

	if (aws_lws_x509_create(&pub)) {
		aws_lwsl_err("%s: failed to create x509 public key\n", __func__);
		goto bail;
	}

	if (aws_lws_x509_parse_from_pem(pub, rsa_cert, LWS_ARRAY_SIZE(rsa_cert))) {
		aws_lwsl_err("%s: failed to parse x509 public key\n", __func__);
		goto bail;
	}

	if (aws_lws_x509_public_to_jwk(&jwk, pub, NULL, 2048)) {
		aws_lwsl_err("%s: failed to copy public key to jwk\n", __func__);
		goto bail;
	}

	if (aws_lws_x509_jwk_privkey_pem(context, &jwk, (char *)rsa_key,
				     LWS_ARRAY_SIZE(rsa_key), NULL)) {
		aws_lwsl_err("%s: failed to copy private key to jwk\n", __func__);
		goto bail;
	}

	if (aws_lws_x509_info(pub, LWS_TLS_CERT_INFO_DER_RAW, der_info,
	...
	if (!aws_lws_SHA1((unsigned char *)der_info->ns.name,
		      (size_t)der_info->ns.len, sha1sum)) {
		aws_lwsl_err("%s: sha1sum of public key failed\n", __func__);
		goto bail;
	}

	ret_encode = aws_lws_b64_encode_string_url((char *)sha1sum,
				LWS_ARRAY_SIZE(sha1sum), sha1_fingerprint,
				LWS_ARRAY_SIZE(sha1_fingerprint));
	if (ret_encode < 0) {
		aws_lwsl_err("%s: failed to encode sha1sum to base64url\n", __func__);
		goto bail;
	}
	...
	aws_lwsl_notice("%s: cert fingerprint '%s'\n", __func__, sha1_fingerprint);
	...
	/* now produce jwt with some additional header fields */
	{
		...
		struct aws_lws_jwt_sign_info info = {
			.alg = NULL,
			.jose_hdr = jose_hdr,
			.jose_hdr_len = (size_t)aws_lws_snprintf(jose_hdr, LWS_ARRAY_SIZE(jose_hdr), hdr_fmt, sha1_fingerprint),
			.out = buf,
			.out_len = &cml2,
			.temp = buf + cml2,
			.tl = 4096
		...
		aws_lwsl_notice("%s: jose_hdr of len %zu: '%s'\n", __func__, info.jose_hdr_len, info.jose_hdr);
		if (aws_lws_jwt_sign_via_info(context, &jwk, &info,
					"{\"iss\":\"warmcat.com\",\"aud\":"
					"\"https://libwebsockets.org/sai\","
					"\"iat\":%llu,"
					"\"nbf\":%llu,"
		...
		if (aws_lws_jwt_signed_validate(context, &jwk, "RS256",
							 (const char *)buf, cml2,
							 (char *)buf + 2048, 2048,
							 (char *)buf + 4096, &cml)) {
			aws_lwsl_err("%s: failed to parse JWT\n", __func__);

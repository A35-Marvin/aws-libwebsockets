...
static struct aws_lws_poly_gen tx = { { 0xabcde, 0x23456789 } },
...
static uint8_t
aws_lws_poly_rand(struct aws_lws_poly_gen *p)
{
			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_CLIENT_ESTABLISHED:
		aws_lwsl_info("dumb: LWS_CALLBACK_CLIENT_ESTABLISHED\n");
		break;

	case LWS_CALLBACK_CLOSED:
		aws_lwsl_notice("dumb: LWS_CALLBACK_CLOSED\n");
		wsi_dumb = NULL;
		break;

	case LWS_CALLBACK_CLIENT_RECEIVE:
		((char *)in)[len] = '\0';
		aws_lwsl_info("rx %d '%s'\n", (int)len, (char *)in);
		break;
	...
	case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
		...
		aws_lwsl_err("CLIENT_CONNECTION_ERROR: %s: %s\n", which,
			 in ? (char *)in : "(null)");
		break;
	...
	case LWS_CALLBACK_CLIENT_CONFIRM_EXTENSION_SUPPORTED:
		if ((strcmp((const char *)in, "deflate-stream") == 0) &&
		    deny_deflate) {
			aws_lwsl_notice("denied deflate-stream extension\n");
			return 1;
		}
	...
	case LWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ:
		aws_lwsl_notice("LWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ: %ld\n",
			    (long)len);
	...
	case LWS_CALLBACK_CLIENT_WRITEABLE:
		aws_lwsl_info("Client wsi %p writable\n", wsi);
		break;
	...
	case LWS_CALLBACK_CLIENT_HTTP_WRITEABLE:
		 ...
		 * if we had more to send, call aws_lws_callback_on_writable(wsi);
...
#endif
							 n, errbuf);
				aws_lwsl_err("EXTRA_CLIENT_VERIFY_CERTS: "
					 "SSL error: %s (%d)\n", es, n);
				return 1;
			}
		}
		    ...
		    void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_CLIENT_ESTABLISHED:
		...
		aws_lwsl_notice("mirror: LWS_CALLBACK_CLIENT_ESTABLISHED\n");
		...
		aws_lws_get_random(aws_lws_get_context(wsi), rands, sizeof(rands[0]));
		mirror_lifetime = (int)(16384 + (rands[0] & 65535));
		/* useful to test single connection stability */
		if (longlived)
			mirror_lifetime += 500000;

		aws_lwsl_notice("opened mirror connection with "
			  "%d lifetime\n", mirror_lifetime);
	...
	case LWS_CALLBACK_CLIENT_CLOSED:
		aws_lwsl_notice("mirror: LWS_CALLBACK_CLOSED mirror_lifetime=%d, "
			    "rxb %d, rx_count %d\n", mirror_lifetime, rxb,
			    rx_count);
		wsi_mirror = NULL;
		if (flag_echo || once)
	...
	case LWS_CALLBACK_CLIENT_WRITEABLE:
		...
		if (flag_echo) {
			for (n = 0; n < (int)block_size; n++)
				buf[LWS_PRE + n] = aws_lws_poly_rand(&tx);

			n = aws_lws_write(wsi, &buf[LWS_PRE], block_size,
				      opts | LWS_WRITE_TEXT);
			if (n < 0) {
				aws_lwsl_err("Error sending\n");
				return -1;
			}
			...
			if (txb != count_blocks)
				...
				aws_lwsl_notice("send completed: %d x %d\n",
		...
		for (n = 0; n < 1; n++) {
			aws_lws_get_random(aws_lws_get_context(wsi), rands,
				       sizeof(rands));
		...
		n = (int)aws_lws_write(wsi, &buf[LWS_PRE], (unsigned int)l,
			      opts | LWS_WRITE_TEXT);
		if (n < 0)
			return -1;
		if (n < l) {
			aws_lwsl_err("Partial write LWS_CALLBACK_CLIENT_WRITEABLE\n");
			return -1;
		}
		if (!justmirror)
			mirror_lifetime--;
		if (!mirror_lifetime) {
			aws_lwsl_notice("closing mirror session\n");
			return -1;
		}
	...
	case LWS_CALLBACK_CLIENT_RECEIVE:
		if (flag_echo) {
			p = (unsigned char *)in;
			for (n = 0; n < (int)len; n++)
				if (*p++ != aws_lws_poly_rand(&rx)) {
					aws_lwsl_err("mismatch at rxb %d offset %d\n", (int)rxb + (n / block_size), n % block_size);
					errs++;
					force_exit = 1;
					return -1;
				}
			...
			if (rx_count == 0 && rxb == count_blocks) {
				aws_lwsl_notice("Everything received: errs %d\n",
					    errs);
				force_exit = 1;
				return -1;
			}
...
int main(int argc, char **argv)
{
	...
	char key_path[1024] = "";
	char ca_path[1024] = "";
	unsigned long last = aws_lws_now_secs();

	memset(&info, 0, sizeof info);

	aws_lwsl_notice("libwebsockets test client - license MIT\n");
	...
	while (n >= 0) {
       ...
       n = getopt(argc, argv, "Sjnuv:hsp:d:lC:K:A:moeO");
		...
		switch (n) {
		case 'd':
			aws_lws_set_log_level(atoi(optarg), NULL);
			break;
		...
		case 'n':
			...
			aws_lwsl_notice("Disabled sending mirror data (for pingpong testing)\n");
			break;
		case 'C':
	...
	i.port = port;
	if (aws_lws_parse_uri(argv[optind], &prot, &i.address, &i.port, &p))
		goto usage;
	...
	if (use_ssl) {
		...
		if (ca_path[0])
			...
			aws_lwsl_notice("WARNING, providing a CRL requires a CA cert!\n");
#endif
	}

	if (use_ssl & LCCSCF_USE_SSL) {
		aws_lwsl_notice(" Using SSL\n");
#if defined(LWS_WITH_MBEDTLS)
		aws_lwsl_notice("   (NOTE: mbedtls needs to be given the remote\n");
		aws_lwsl_notice("    CA cert to trust (with -A) to validate it)\n");
#endif
	}
	else
		aws_lwsl_notice(" SSL disabled\n");
	if (use_ssl & LCCSCF_ALLOW_SELFSIGNED)
		aws_lwsl_notice(" Selfsigned certs allowed\n");
	else
		aws_lwsl_notice(" Cert must validate correctly (use -s to allow selfsigned)\n");
	if (use_ssl & LCCSCF_SKIP_SERVER_CERT_HOSTNAME_CHECK)
		aws_lwsl_notice(" Skipping peer cert hostname check\n");
	else
		aws_lwsl_notice(" Requiring peer cert hostname matches\n");
#endif
	...
	if (!strcmp(prot, "http") || !strcmp(prot, "https")) {
		aws_lwsl_notice("using %s mode (non-ws)\n", prot);
		if (test_post) {
			i.method = "POST";
			aws_lwsl_notice("POST mode\n");
		}
		else
			i.method = "GET";
		do_ws = 0;
		...
		if (!strcmp(prot, "raw")) {
			...
			aws_lwsl_notice("using RAW mode connection\n");
			do_ws = 0;
		} else
			aws_lwsl_notice("using %s mode (ws)\n", prot);
	 ...
	 * asynchronously established... calling aws_lws_client_connect() only
	 * instantiates the connection logically, aws_lws_service() progresses it
	 * asynchronously.
	 */
		...
		if (do_multi) {
			for (n = 0; n < (int)LWS_ARRAY_SIZE(wsi_multi); n++) {
				if (!wsi_multi[n] && ratelimit_connects(&rl_multi[n], 2u)) {
					aws_lwsl_notice("dumb %d: connecting\n", n);
			...
			if (do_ws) {
				if (!flag_echo && !justmirror && !wsi_dumb && ratelimit_connects(&rl_dumb, 2u)) {
					aws_lwsl_notice("dumb: connecting\n");
					i.protocol = protocols[PROTOCOL_DUMB_INCREMENT].name;
					i.pwsi = &wsi_dumb;
					aws_lws_client_connect_via_info(&i);
				}
				...
				if (!wsi_mirror && ratelimit_connects(&rl_mirror, 2u)) {
					aws_lwsl_notice("mirror: connecting\n");
					i.protocol = protocols[PROTOCOL_LWS_MIRROR].name;
					i.pwsi = &wsi_mirror;
					wsi_mirror = aws_lws_client_connect_via_info(&i);
				}
			} else
				if (!wsi_dumb && ratelimit_connects(&rl_dumb, 2u)) {
					aws_lwsl_notice("http: connecting\n");
					i.pwsi = &wsi_dumb;
					aws_lws_client_connect_via_info(&i);
				}
		}

		aws_lws_service(context, 500);

		if (do_multi) {
			m++;
			if (m == 10) {
				m = 0;
				aws_lwsl_notice("doing lws_callback_on_writable_all_protocol\n");
				aws_lws_callback_on_writable_all_protocol(context,
					   &protocols[PROTOCOL_DUMB_INCREMENT]);
			}
		}

		if (flag_echo && aws_lws_now_secs() != last) {
			aws_lwsl_notice("rxb %d, rx_count %d\n", rxb, rx_count);
			last = aws_lws_now_secs();
		}
	}

	aws_lwsl_err("Exiting\n");

...
int
aws_lws_ss_check_next_state_sspc(aws_lws_sspc_handle_t *ss, uint8_t *prevstate,
			     aws_lws_ss_constate_t cs)
{
	...
	if (cs >= LWS_ARRAY_SIZE(ss_state_txn_validity)) {
		...
		aws_lwsl_sspc_err(ss, "bad new state %u", cs);
		assert(0);
		return 1;
	}
	...
	if (*prevstate >= LWS_ARRAY_SIZE(ss_state_txn_validity)) {
		...
		aws_lwsl_sspc_err(ss, "bad existing state %u",
	...
	aws_lwsl_sspc_err(ss, "transition from %s -> %s is illegal",
		    aws_lws_ss_state_name((int)*prevstate),
		    aws_lws_ss_state_name((int)cs));
...
aws_lws_sspc_event_helper(aws_lws_sspc_handle_t *h, aws_lws_ss_constate_t cs,
		      aws_lws_ss_tx_ordinal_t flags)
{
	aws_lws_ss_state_return_t ret;

	if (!h)
		return LWSSSSRET_OK;

	if (aws_lws_ss_check_next_state_sspc(h, &h->prev_ss_state, cs))
...
static void
aws_lws_sspc_sul_retry_cb(aws_lws_sorted_usec_list_t *sul)
{
	aws_lws_sspc_handle_t *h = aws_lws_container_of(sul, aws_lws_sspc_handle_t, sul_retry);
	...
	i.origin = i.address;
	i.method = "RAW";
	i.protocol = aws_lws_sspc_protocols[0].name;
	i.local_protocol_name = aws_lws_sspc_protocols[0].name;
	i.path = "";
	i.pwsi = &h->cwsi;
	i.opaque_user_data = (void *)h;
	i.ssl_connection = LCCSCF_SECSTREAM_PROXY_LINK;

	aws_lws_metrics_caliper_bind(h->cal_txn, h->context->mt_ss_cliprox_conn);
#if defined(LWS_WITH_SYS_METRICS)
	aws_lws_metrics_tag_add(&h->cal_txn.mtags_owner, "ss", h->ssi.streamtype);
#endif
	...
	if (!aws_lws_client_connect_via_info(&i)) {
		...
		aws_lws_sul_schedule(h->context, 0, &h->sul_retry,
				 aws_lws_sspc_sul_retry_cb, LWS_US_PER_SEC);

		return;
	}

	aws_lwsl_sspc_notice(h, "%s", h->cwsi->lc.gutag);
}

static int
aws_lws_sspc_serialize_metadata(aws_lws_sspc_handle_t *h, aws_lws_sspc_metadata_t *md,
				uint8_t *p, uint8_t *end)
{
	...
	if (md->name[0] == '\0') {
		...
		aws_lwsl_info("sending tx credit update %d\n",
				md->tx_cr_adjust);

		p[0] = LWSSS_SER_TXPRE_TXCR_UPDATE;
		aws_lws_ser_wu16be(&p[1], 4);
		aws_lws_ser_wu32be(&p[3], (uint32_t)md->tx_cr_adjust);

		n = 7;

	} else {

		aws_lwsl_sspc_info(h, "sending metadata");
		...
		if (n > 0xffff)
			...
			return -1;
		if (n > aws_lws_ptr_diff(end, &p[4]))
			/* we don't have space for this metadata */
			return -1;
		aws_lws_ser_wu16be(&p[1], (uint16_t)n);
		p[3] = (uint8_t)txc;
		     ...
		     void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
		...
		aws_lws_sul_schedule(h->context, 0, &h->sul_retry,
				 aws_lws_sspc_sul_retry_cb, LWS_US_PER_SEC);
		if (h->ssi.state) {
			interval = (uint64_t)(aws_lws_now_usecs() - h->us_start_upstream) /
								LWS_US_PER_MS;
			if (interval > 0xffffffffull)
				interval = 0xffffffffull;
			r = h->ssi.state(aws_lws_sspc_to_user_object(h), NULL,
					  LWSSSCS_UPSTREAM_LINK_RETRY,
					  (uint32_t)interval);
			if (r == LWSSSSRET_DESTROY_ME)
				aws_lws_sspc_destroy(&h);
		}
		break;

        case LWS_CALLBACK_RAW_CONNECTED:
		if (!h || aws_lws_fi(&h->fic, "sspc_fail_on_linkup"))
			return -1;
		aws_lwsl_sspc_info(h, "CONNECTED (%s)", h->ssi.streamtype);
	...
	case LWS_CALLBACK_RAW_CLOSE:
		 ...
		 */
		if (!h) {
			aws_lwsl_info("%s: no sspc on client proxy link close", __func__);
			break;
		}
		aws_lwsl_sspc_info(h, "LWS_CALLBACK_RAW_CLOSE: proxy conn down, wsi %s",
				aws_lws_wsi_tag(wsi));

		aws_lws_dsh_destroy(&h->dsh);
		if (h->ss_dangling_connected && h->ssi.state) {

			aws_lwsl_sspc_notice(h, "setting _DISCONNECTED");
	...
	case LWS_CALLBACK_RAW_RX:
		...
		if (!h || !h->cwsi) {
			aws_lwsl_info("%s: rx when client ss destroyed\n", __func__);

			return -1;
		}

		aws_lwsl_sspc_info(h, "%s: RAW_RX: rx %d\n", __func__, (int)len);

		if (!len) {
			aws_lwsl_sspc_notice(h, "RAW_RX: zero len");

			return -1;
		}

		if (aws_lws_fi(&h->fic, "sspc_fake_rxparse_disconnect_me"))
			n = LWSSSSRET_DISCONNECT_ME;
		else
			if (aws_lws_fi(&h->fic, "sspc_fake_rxparse_destroy_me"))
				n = LWSSSSRET_DESTROY_ME;
			else
				n = aws_lws_ss_deserialize_parse(&h->parser,
							     aws_lws_get_context(wsi),
							     h->dsh, in, len,
							     &h->state, h,
							     (aws_lws_ss_handle_t **)m,
							     &h->ssi, 1);
		switch (n) {
		case LWSSSSRET_OK:
			break;
		case LWSSSSRET_DISCONNECT_ME:
			aws_lwsl_info("%s: proxlicent RX ended with DISCONNECT_ME\n",
					__func__);
			return -1;
		case LWSSSSRET_DESTROY_ME:
			aws_lwsl_info("%s: proxlicent RX ended with DESTROY_ME\n",
					__func__);
			aws_lws_set_opaque_user_data(wsi, NULL);
			aws_lws_sspc_destroy(&h);
			return -1;
	...
	case LWS_CALLBACK_RAW_WRITEABLE:
		...
		aws_lwsl_sspc_debug(h, "WRITEABLE %s, state %d",
		...
		if (h->pending_timeout_update) {
			 ...
			 */
			aws_lws_ser_wu32be(&s[3], h->timeout_ms);
		...
		case LPCSCLI_SENDING_INITIAL_TX:
			...
			aws_lws_ser_wu16be(&s[1], (uint16_t)n);
			/* SSSv1: add protocol version byte (initially 1) */
			s[3] = (uint8_t)LWS_SSS_CLIENT_PROTOCOL_VERSION;
			aws_lws_ser_wu32be(&s[4], (uint32_t)getpid());
			aws_lws_ser_wu32be(&s[8], (uint32_t)h->txc.peer_tx_cr_est);
			//h->txcr_out = txc;
			aws_lws_strncpy((char *)&s[12], h->ssi.streamtype, sizeof(s) - 12);
			n += 3;
		...
		case LPCSCLI_LOCAL_CONNECTED:
			...
			// aws_lwsl_notice("%s: LPCSCLI_LOCAL_CONNECTED\n", __func__);
			...
			if (h->metadata_owner.count) {
				aws_lws_sspc_metadata_t *md = aws_lws_container_of(
					aws_lws_dll2_get_tail(&h->metadata_owner),
					aws_lws_sspc_metadata_t, list);

				pkt = aws_lws_malloc(pktsize + LWS_PRE, __func__);
				if (!pkt)
					goto hangup;
				cp = p = pkt + LWS_PRE;
				end = p + pktsize;

				n = aws_lws_sspc_serialize_metadata(h, md, p, end);
				if (n < 0)
					goto metadata_hangup;

				aws_lwsl_sspc_debug(h, "(local_conn) metadata");

				goto req_write_and_issue;
			}

			if (h->pending_writeable_len) {
				aws_lwsl_sspc_debug(h, "(local_conn) PAYLOAD_LENGTH_HINT %u",
					   (unsigned int)h->writeable_len);
				s[0] = LWSSS_SER_TXPRE_PAYLOAD_LENGTH_HINT;
				aws_lws_ser_wu16be(&s[1], 4);
				aws_lws_ser_wu32be(&s[3], (uint32_t)h->writeable_len);
			...
			if (h->conn_req_state >= LWSSSPC_ONW_ONGOING) {
				aws_lwsl_sspc_info(h, "conn_req_state %d",
						h->conn_req_state);
				break;
			}

			aws_lwsl_sspc_info(h, "(local_conn) onward connect");
		...
		case LPCSCLI_OPERATIONAL:
			...
			if (h->pending_writeable_len) {
				aws_lwsl_sspc_info(h, "PAYLOAD_LENGTH_HINT %u",
					  (unsigned int)h->writeable_len);
				s[0] = LWSSS_SER_TXPRE_PAYLOAD_LENGTH_HINT;
				aws_lws_ser_wu16be(&s[1], 4);
				aws_lws_ser_wu32be(&s[3], (uint32_t)h->writeable_len);
			...
			if (!h->ignore_txc && h->txc.tx_cr <= 0) {
				aws_lwsl_sspc_info(h, "WRITEABLE / OPERATIONAL:"
					    " lack credit (%d)",
					    h->txc.tx_cr);
				// break;
			}
			...
			case LWSSSSRET_DISCONNECT_ME:
			case LWSSSSRET_DESTROY_ME:
				aws_lwsl_notice("%s: sspc tx DISCONNECT/DESTROY unimplemented\n", __func__);
				break;
			default:
				break;
			}
			...
			aws_lws_ser_wu16be(&p[1], (uint16_t)(len + 19 - 3));
			aws_lws_ser_wu32be(&p[3], (uint32_t)flags);
			/* time spent here waiting to send this */
			aws_lws_ser_wu32be(&p[7], (uint32_t)(us - h->us_earliest_write_req));
			/* ust that the client write happened */
			aws_lws_ser_wu64be(&p[11], (uint64_t)us);
...
do_write:
		if (aws_lws_fi(&h->fic, "sspc_link_write_fail"))
			n = -1;
		else
			n = aws_lws_write(wsi, (uint8_t *)cp, (unsigned int)n, LWS_WRITE_RAW);
		if (n < 0) {
			aws_lwsl_sspc_notice(h, "WRITEABLE: %d", n);
...
metadata_hangup:
	aws_lwsl_sspc_err(h, "metadata too large");

hangup:
	aws_lws_free(pkt);
	aws_lwsl_warn("hangup\n");
	/* hang up on him */
	return -1;
...
const struct aws_lws_protocols aws_lws_sspc_protocols[] = {
	{
		"ssproxy-protocol",
...
aws_lws_sspc_create(struct aws_lws_context *context, int tsi, const aws_lws_ss_info_t *ssi,
	        void *opaque_user_data, aws_lws_sspc_handle_t **ppss,
	        struct aws_lws_sequencer *seq_owner, const char **ppayload_fmt)
{
	...
	h = malloc(sizeof(aws_lws_sspc_handle_t) + ssi->user_alloc +
	...
	if (aws_lws_fi(&h->fic, "sspc_create_oom")) {
		/*
	...
	h->context = context;
	h->us_start_upstream = aws_lws_now_usecs();
	...
	aws_lws_dll2_add_head(&h->client_list, &context->pt[tsi].ss_client_owner);
	...
	*((void **)(ua + ssi->handle_offset)) = h;
	...
	aws_lws_sspc_sul_retry_cb(&h->sul_retry);
...
int
aws_lws_sspc_destroy_dll(struct aws_lws_dll2 *d, void *user)
{
	aws_lws_sspc_handle_t *h = aws_lws_container_of(d, aws_lws_sspc_handle_t, client_list);
...
void
aws_lws_sspc_rxmetadata_destroy(aws_lws_sspc_handle_t *h)
{
	aws_lws_start_foreach_dll_safe(struct aws_lws_dll2 *, d, d1,
			aws_lws_dll2_get_head(&h->metadata_owner_rx)) {
		aws_lws_sspc_metadata_t *md =
				aws_lws_container_of(d, aws_lws_sspc_metadata_t, list);
...
void
aws_lws_sspc_destroy(aws_lws_sspc_handle_t **ph)
{
	aws_lws_sspc_handle_t *h;
	...
	aws_lws_sul_cancel(&h->sul_retry);
	aws_lws_dll2_remove(&h->client_list);

	if (h->dsh)
		aws_lws_dsh_destroy(&h->dsh);
	if (h->cwsi) {
		aws_lws_set_opaque_user_data(h->cwsi, NULL);
		aws_lws_wsi_close(h->cwsi, LWS_TO_KILL_ASYNC);
		h->cwsi = NULL;
	}
	...
	aws_lws_start_foreach_dll_safe(struct aws_lws_dll2 *, d, d1,
			aws_lws_dll2_get_head(&(*ph)->metadata_owner)) {
		aws_lws_sspc_metadata_t *md =
				aws_lws_container_of(d, aws_lws_sspc_metadata_t, list);
	...
	aws_lws_sspc_rxmetadata_destroy(h);

	aws_lws_sspc_event_helper(h, LWSSSCS_DESTROYING, 0);
	*ph = NULL;
...
aws_lws_ss_state_return_t
aws_lws_sspc_request_tx(aws_lws_sspc_handle_t *h)
{
	if (!h || !h->cwsi)
		return LWSSSSRET_OK;
	...
	if (!h->us_earliest_write_req)
		h->us_earliest_write_req = aws_lws_now_usecs();
...
aws_lws_ss_state_return_t
aws_lws_sspc_request_tx_len(aws_lws_sspc_handle_t *h, unsigned long len)
{
	/*
	...
	aws_lwsl_sspc_notice(h, "setting writeable_len %u", (unsigned int)len);
	h->writeable_len = len;
...
int
aws_lws_sspc_client_connect(aws_lws_sspc_handle_t *h)
{
...
aws_lws_sspc_rideshare(struct aws_lws_sspc_handle *h)
{
	...
	if (h->parser.rideshare[0]) {
		aws_lwsl_sspc_info(h, "parser %s", h->parser.rideshare);
	...
	if (h->rideshare_list[0]) {
		aws_lwsl_sspc_info(h, "tx list %s",
		       ...
		       const void *value, size_t len, int tx_cr_adjust)
{
	...
	aws_lws_start_foreach_dll_safe(struct aws_lws_dll2 *, d, d1,
				   aws_lws_dll2_get_head(&h->metadata_owner)) {
		md = aws_lws_container_of(d, aws_lws_sspc_metadata_t, list);
	...
	if (aws_lws_fi(&h->fic, "sspc_fail_metadata_set"))
		md = NULL;
	else
		md = aws_lws_malloc(sizeof(*md) + len, "set metadata");
	if (!md) {
		aws_lwsl_sspc_err(h, "OOM");
	...
	aws_lws_strncpy(md->name, name, sizeof(md->name));
	md->len = len;
	if (len)
		memcpy(&md[1], value, len);

	aws_lws_dll2_add_tail(&md->list, &h->metadata_owner);

	if (len) {
		aws_lwsl_sspc_info(h, "set metadata %s", name);
		aws_lwsl_hexdump_sspc_info(h, value, len);
	} else
		aws_lwsl_sspc_info(h, "serializing tx cr adj %d",
			    (int)tx_cr_adjust);
...
int
aws_lws_sspc_add_peer_tx_credit(struct aws_lws_sspc_handle *h, int32_t bump)
{
	aws_lws_service_assert_loop_thread(h->context, 0);
...
{
	aws_lws_sspc_start_timeout(h, (unsigned int)-1);
}
...
aws_lws_sspc_change_handlers(struct aws_lws_sspc_handle *h,
	aws_lws_ss_state_return_t (*rx)(void *userobj, const uint8_t *buf, size_t len, int flags),
	aws_lws_ss_state_return_t (*tx)(void *userobj, aws_lws_ss_tx_ordinal_t ord, uint8_t *buf,
...
int
aws_lws_sspc_cancel_notify_dll(struct aws_lws_dll2 *d, void *user)
{
	aws_lws_sspc_handle_t *h = aws_lws_container_of(d, aws_lws_sspc_handle_t, client_list);

	aws_lws_sspc_event_helper(h, LWSSSCS_EVENT_WAIT_CANCELLED, 0);

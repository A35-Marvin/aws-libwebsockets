...
//#define LWS_COOKIE_DEBUG
	...
	#define aws_lwsl_cookie aws_lwsl_notice
#else
	#define aws_lwsl_cookie aws_lwsl_debug
#endif

#define LWS_COOKIE_MAX_CACHE_NAME_LEN	128

#define aws_lws_tolower(_c) (((_c) >= 'A' && (_c) <= 'Z') ? \
					    (char)((_c) + 'a' - 'A') : \
					    (char)(_c))
...
static int
aws_lws_cookie_parse_date(const char *d, size_t len, time_t *t)
{
	struct tm date;
	int offset = 0, i;
	...
	while (len) {
		...
		case 3:
			for (i = 0; i < 36; i += 3) {
				if (aws_lws_tolower(*(d - 3)) == mon[i] &&
				    aws_lws_tolower(*(d - 2)) == mon[i + 1] &&
				    aws_lws_tolower(*(d - 1)) == mon[i + 2]) {
					date.tm_mon = i / 3;
					break;
				}
			}
...
static void
aws_lws_cookie_rm_sws(const char **buf_p, size_t *len_p)
{
	const char *buf;
	size_t len;

	if (!buf_p || !*buf_p || !len_p || !*len_p) {
		aws_lwsl_err("%s: false parameter\n", __func__);
		return;
	}
...
is_iprefix(const char *h, size_t hl, const char *n, size_t nl)
{
	...
	while (nl) {
		nl--;
		if (aws_lws_tolower(h[nl]) != aws_lws_tolower(n[nl]))
			return 0;
	}
	return 1;
}
...
static int
aws_lws_cookie_compile_cache_name(char *buf, size_t buf_len, struct aws_lws_cookie *c)
{
...
static int
aws_lws_cookie_parse_nsc(struct aws_lws_cookie *c, const char *b, size_t l)
{
	enum aws_lws_cookie_nsc_f state = LWSC_NSC_DOMAIN;
	size_t n = 0;
...
aws_lws_cookie_write_nsc(struct aws_lws *wsi, struct aws_lws_cookie *c)
{
	...
	stash = wsi->stash ? wsi->stash : aws_lws_get_network_wsi(wsi)->stash;
	...
	if (!c->f[CE_EXPIRES]) {
		 ...
		 */
		aws_lwsl_warn("%s: reject session cookies\n", __func__);
	...
	if (aws_lws_cookie_compile_cache_name(cache_name, sizeof(cache_name), c))
		return -1;

	if (c->f[CE_EXPIRES] &&
	    aws_lws_cookie_parse_date(c->f[CE_EXPIRES], c->l[CE_EXPIRES], &expires)) {
		aws_lwsl_err("%s: can't parse date %.*s\n", __func__,
			 (int)c->l[CE_EXPIRES], c->f[CE_EXPIRES]);
		return -1;
	}

	size += c->l[CE_NAME] + c->l[CE_VALUE] + c->l[CE_DOMAIN] + c->l[CE_PATH];
	cookie_string = (char *)aws_lws_malloc(size, __func__);
	if (!cookie_string) {
		aws_lwsl_err("%s: OOM\n",__func__);
	...
	aws_lws_snprintf(cookie_string, size, LWS_COOKIE_NSC_FORMAT,
	...
	aws_lwsl_cookie("%s: name %s\n", __func__, cache_name);
	aws_lwsl_cookie("%s: c %s\n", __func__, cookie_string);

	if (aws_lws_cache_write_through(l1, cache_name,
				    (const uint8_t *)cookie_string,
				    strlen(cookie_string),
				    (aws_lws_usec_t)((unsigned long long)expires *
					   (aws_lws_usec_t)LWS_US_PER_SEC), NULL)) {
		ret = -1;
		goto exit;
	}
...
#if defined(LWS_COOKIE_DEBUG)
	char *po;
	if (aws_lws_cache_item_get(l1, cache_name, (const void **)&po, &size) ||
	    size != strlen(cookie_string) || memcmp(po, cookie_string, size)) {
		aws_lwsl_err("%s: L1 '%s' missing\n", __func__, cache_name);
	}

	if (aws_lws_cache_item_get(wsi->a.context->nsc, cache_name,
			       (const void **)&po, &size) ||
...
aws_lws_cookie_attach_cookies(struct aws_lws *wsi, char *buf, char *end)
{
	...
	if (dl_path)
		path_len = aws_lws_ptr_diff_size_t(dl_path,  path);
	...
	while (dl_domain) {
		...
		*p1 = '\0';
		...
		aws_lwsl_cookie("%s: looking for %s\n", __func__, cache_name);

		if (!aws_lws_cache_lookup(l1, cache_name,
			...
			while (!aws_lws_cache_results_walk(&cr)) {
				...
				if (aws_lws_cache_item_get(l1, (const char *)cr.tag,
						   (const void **)&po, &size) ||
					aws_lws_cookie_parse_nsc(&c, po, size)) {
					aws_lwsl_err("%s: failed to get c '%s'\n",
							__func__, cr.tag);
					break;
				}

				if (c.f[CE_HOSTONLY] && !hostdomain){
					aws_lwsl_cookie("%s: not sending this\n",
							__func__);
					continue;
				}
...
aws_lws_parse_set_cookie(struct aws_lws *wsi)
{
	...
	while (f) {
...
#if defined(LWS_COOKIE_DEBUG)
		aws_lwsl_notice("%s:parsing: %.*s\n", __func__, (int)fl, cookiep);
#endif

		buf_head = cookiep;
		buf_end = cookiep + fl - 1;
		memset(&c, 0, sizeof(struct aws_lws_cookie));
		...
		do {
			...
			dl = memchr(tk_head, '=', aws_lws_ptr_diff_size_t(tk_end,
							tk_head + 1));
			if (!dl || dl == tk_head)
				return -1;

			c.f[CE_NAME] = tk_head;
			c.l[CE_NAME] = aws_lws_ptr_diff_size_t(dl, tk_head);
			aws_lws_cookie_rm_sws(&c.f[CE_NAME], &c.l[CE_NAME]);

			if (!c.l[CE_NAME])
				return -1;

			aws_lwsl_cookie("%s: c name l %d v:%.*s\n", __func__,
					(int)c.l[CE_NAME],
					(int)c.l[CE_NAME], c.f[CE_NAME]);
			c.f[CE_VALUE] = dl + 1;
			c.l[CE_VALUE] = aws_lws_ptr_diff_size_t(tk_end,
						   c.f[CE_VALUE]) + 1;

			aws_lws_cookie_rm_sws(&c.f[CE_VALUE], &c.l[CE_VALUE]);
			if (c.l[CE_VALUE] >= 2 && c.f[CE_VALUE][0] == '\"') {
				c.f[CE_VALUE]++;
				c.l[CE_VALUE] -= 2;
			}
			aws_lwsl_cookie("%s: c value l %d v:%.*s\n", __func__,
...
parse_av:
			...
			for (n = 0; n < (int)LWS_ARRAY_SIZE(cft); n++) {
				if (aws_lws_tolower(*tk_head) != cft[n].name[0])
					continue;

				if (!is_iprefix(tk_head,
						aws_lws_ptr_diff_size_t(tk_end,
								   tk_head) + 1,
				...
				c.l[n] = aws_lws_ptr_diff_size_t(tk_end, c.f[n]) + 1;
				aws_lws_cookie_rm_sws(&c.f[n], &c.l[n]);
		...
		if (aws_lws_cookie_write_nsc(wsi, &c))
			aws_lwsl_err("%s:failed to write nsc\n", __func__);
	}
...
aws_lws_cookie_send_cookies(struct aws_lws *wsi, char **pp, char *end)
{
	...
	size = aws_lws_cookie_attach_cookies(wsi, NULL, NULL);

	if (!size)
		return 0;
	if (size < 0) {
		aws_lwsl_err("%s:failed to get cookie string size\n", __func__);
		return -1;
	}

	aws_lwsl_notice("%s: size %d\n", __func__, size);

#if defined(LWS_COOKIE_DEBUG)
		char *p_dbg = *pp;
#endif

	if (aws_lws_add_http_header_by_token(wsi, WSI_TOKEN_HTTP_COOKIE, NULL, size,
								(unsigned char **)pp, (unsigned char *)end))
		return -1;

#if defined(LWS_COOKIE_DEBUG)
		aws_lwsl_notice("%s: dummy copy (%.*s) \n", __func__, (int)(*pp - p_dbg), p_dbg);
#endif


#ifdef LWS_WITH_HTTP2
	if (aws_lws_wsi_is_h2(wsi))
		p = *pp - size;
	else
#endif
		p = *pp - size - 2;

	if (aws_lws_cookie_attach_cookies(wsi, p, p + size) <= 0) {
		aws_lwsl_err("%s:failed to attach cookies\n", __func__);
		return -1;
	}

#if defined(LWS_COOKIE_DEBUG)
		aws_lwsl_notice("%s: real copy (%.*s) total len %d\n", __func__, (int)(*pp - p_dbg), p_dbg, (int)(*pp - p_dbg));
		aws_lwsl_hexdump_notice(p_dbg, (size_t)(*pp - p_dbg));
#endif

			...
			void *user, void *in, size_t len)
{
	...
	const struct aws_lws_protocol_vhost_options *opt;
	int n, m;
	...
	case LWS_CALLBACK_ESTABLISHED:
		...
		if (!vhd->options || !((*vhd->options) & 1))
			aws_lws_set_timer_usecs(wsi, DUMB_PERIOD_US);
		break;

	case LWS_CALLBACK_SERVER_WRITEABLE:
		n = aws_lws_snprintf((char *)p, sizeof(buf) - LWS_PRE, "%d",
				 pss->number++);
		m = aws_lws_write(wsi, p, (unsigned int)n, LWS_WRITE_TEXT);
		if (m < n) {
...
LWS_VISIBLE const struct aws_lws_protocols dumb_increment_protocols[] = {
	LWS_PLUGIN_PROTOCOL_DUMB_INCREMENT
};

LWS_VISIBLE const aws_lws_plugin_protocol_t dumb_increment = {
	.hdr = {
		"dumb increment",
